...
int main(int argc, char *argv[])
{
	...
	if (n == 1) {
		...
		while (fgets(line, sizeof(line), f) != NULL) {
			...
			if (len > 0 && line[len - 1] == '\n') {
				line[len - 1] = '\0';
				--len; // besoin d'etre clair sur ca  --len
			}
			...
			if (value != NULL) {
				if (value->frequency == value->capacity) {
					if (value->capacity * sizeof(long int) > SIZE_MAX / LINES__CAPACITY_MUL)
						goto error_capacity;
					long int *a = realloc(value->lines, value->capacity * LINES__CAPACITY_MUL * sizeof(long int));
					if (a == NULL)
						goto error_capacity;
					value->lines = a;
				...
				if (len > 0) {
					...
					value->lines = malloc(LINES__CAPACITY_MIN * sizeof *(value->lines));
					if (value->lines == NULL) {
						free(value);
						goto error_capacity;
					}
		...
		char *prev_filename = NULL; //pour savoir est-ce -qu'on a passer d'un ficher a un autre
		for (int i = optind; i < n; ++i) {
			...
			while (fgets(line, sizeof(line), f) != NULL) {
				...
				if (value != NULL) {
					...
					if (strcmp(filename, prev_filename) != 0){
						(value->frequency)++; // incrémenter la fréquence si on a changer de fichier
					}
